...
`build\Release.ps1` embeds these files into `SEImageConverter-Setup-<version>.exe` by default. During setup, the installer checks for the required .NET Desktop Runtime and Windows App Runtime, then runs the bundled installer only when the runtime is missing.
